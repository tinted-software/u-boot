...
static struct mach_o_load_info load_mach_o_image(void *mach_o_image) {
  ...
  for (i = 0; i < header->commands_nb; ++i) {
    if (lc->command == LOAD_COMMAND_SEGMENT) {
      ...
      ret.end = max_t(uintptr_t, sc->dst + sc->dst_len, ret.end);
  ...
  for (i = 0; i < header->commands_nb; ++i) {
    if (lc->command == LOAD_COMMAND_SEGMENT) {
      ...
      if (sc->src_len != sc->dst_len && sc->dst_len)
        memset(dst + sc->src_len, 0, sc->dst_len - sc->src_len);
...
int do_bootxnu(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[]) {
  ...
  if (vid_priv) {
    ...
    boot_args->video_information.bytes_per_row =
        ((1 << vid_priv->bpix) * (vid_priv->xsize)) / 8;
  ...
  dcache_disable();
#ifdef CONFIG_ARM64
  armv8_switch_to_el1((u64)boot_args, 0, 0, 0, (u64)xnu_entry, ES_TO_AARCH64);
#else
...
U_BOOT_CMD(
    bootxnu, 3, 0, do_bootxnu, "Boot XNU from a Mach-O image\n",
